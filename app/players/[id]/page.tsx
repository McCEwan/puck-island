...
export default function PlayerDetailPage() {
  ...
  const [warBySeason, setWarBySeason] = useState<Record<string, WARComponents | null>>({});
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function load() {
      const [playerRes, statsRes, warRes] = await Promise.all([
        fetch(`/api/players/${id}`),
        fetch(`/api/players/${id}/stats`),
        fetch(`/api/players/${id}/war`),
      ]);
      ...
      if (warRes.ok) {
        ...
        setWarBySeason(warData.seasons ?? {});
      }
      setLoading(false);
    }
    load();
  ...
  const warChartData = orderedStats
    .filter(s => warBySeason[s.season_id])
    .map(s => ({
      season: s.season_id,
      WAR:    warBySeason[s.season_id]!.war,
    }));
  ...
  const careerWAR = orderedStats.reduce((sum, s) => sum + (warBySeason[s.season_id]?.war ?? 0), 0);
      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: "20px 16px 80px", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div className="card" style={{ padding: 32 }}>
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            ...
            {warChartData.length > 0 && (
              <span className="pill" style={{ background: careerWAR >= 0 ? "#4ade8015" : "#f8717115", color: careerWAR >= 0 ? "#4ade80" : "#f87171" }}>
                Career WAR: {careerWAR.toFixed(1)}
              </span>
            )}
          </div>
        </div>
            ...
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={warChartData}>
                ...
                <Tooltip
                  ...
                  formatter={(value: any) => [value.toFixed(1), "WAR"]}
                />
                <Bar dataKey="WAR" radius={[4, 4, 4, 4]}>
                  {warChartData.map((d, i) => (
                    <Cell key={i} fill={d.WAR >= 0 ? "#4ade80" : "#f87171"} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        <div className="card" style={{ overflow: "hidden" }}>
          <div style={{ padding: "20px 24px 0" }}>
            <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 24, marginBottom: 4 }}>Year by Year</div>
            <div style={{ fontSize: 12, color: "#64748b", marginBottom: 16 }}>
              Off/Def/PP/PK/Fin GAR are the goals-above-replacement components that sum to WAR (via the wins-per-goal constant) — blank where a season didn't qualify (min 30 GP)
            </div>
          </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
              <thead>
                <tr style={{ background: "#111c2d", color: "#64748b" }}>
                  {["Season","Team","GP","G","A","PTS","Shots","SH%","PPG","Off GAR","Def GAR","PP GAR","PK GAR","Fin GAR","WAR"].map(h => (
                    <th key={h} style={{ padding: "12px 16px", fontWeight: 600, fontSize: 12, textTransform: "uppercase", letterSpacing: ".06em", textAlign: "left", whiteSpace: "nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {[...orderedStats].reverse().map(s => {
                  const w = warBySeason[s.season_id];
                  return (
                    <tr key={s.season_id} className="tr-hover" style={{ borderTop: "1px solid #1e2d40" }}>
                      ...
                      <td style={{ color: "#94a3b8" }}>{w ? w.offenseGAR.toFixed(1)   : "—"}</td>
                      <td style={{ color: "#94a3b8" }}>{w ? w.defenseGAR.toFixed(1)   : "—"}</td>
                      <td style={{ color: "#94a3b8" }}>{w ? w.ppGAR.toFixed(1)        : "—"}</td>
                      <td style={{ color: "#94a3b8" }}>{w ? w.pkGAR.toFixed(1)        : "—"}</td>
                      <td style={{ color: "#94a3b8" }}>{w ? w.finishingGAR.toFixed(1) : "—"}</td>
                      <td style={{ fontWeight: 800, color: !w ? "#475569" : w.war >= 0 ? "#4ade80" : "#f87171" }}>
                        {w ? w.war.toFixed(1) : "—"}
                      </td>
                    </tr>
                  );
